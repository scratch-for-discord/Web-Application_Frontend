...
export default {
    ...
    computed: {
        configurationValidated: function () {
            return this.$store.state.workspace && this.$store.state.workspace.getAllBlocks().some((block) => block.type === "s4d_login") && this.$store.state.workspace.getAllBlocks().every((block) => !block.disabled && !block.warning);
        }
    },
    mounted() {
        document.getElementById("docName").addEventListener(
            "input",
            function (event) {
                document.title = `Scratch For Discord - ${event.target.textContent}`;
                if (typeof window.ScratchNative?.setWorkspaceTitle === "function") {
                    window.ScratchNative.setWorkspaceTitle(event.target.textContent);
                }
            },
            false
        );
